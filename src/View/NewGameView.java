...
public class GameViggewManger implements Observer{
    ...
    private AnimationTimer gametimer, boomTimer,extraTimer;
    ...
    private List<SPECIAL> CurrentSpecial = new LinkedList<SPECIAL>();
    private int tmp=3;
    public int numberOfLifes=3,flag=0;;

    private int difficltyLevel=1;
    private  int boomsLevel=0;
    private  int ExtratimeLevel=1;
    ...
    private String EXTRATIME_PATH,TEXT;
    private List<ImageView> extratime = new LinkedList<ImageView>();
    private List<ExtraTime> TimeObject = new LinkedList<ExtraTime>();
    ...
    private void resetFunction(){
        ...
        for(int i=0; i<extratime.size();i++) {
            gamePane.getChildren().remove(extratime.get(i));
            extratime.remove(extratime.get(i));
            CurrentSpecial.remove(CurrentSpecial.get(i));
            TimeObject.remove(TimeObject.get(i));
        }  
       }
...
//            time.reset();
    ...
    private void createCaseloop(int Case){
            ...
            public void handle(long l) {
              ...
              //  setDiffculty();
                ...
                if(Case==0){
                checkLife();
              //  extratime.remove(CurrentSpecial);
                }
            }
        };
            gametimer.start();
    ...
    private void setNewElementPositionSPECIAL(ImageView image){
        image.setLayoutY(gameEngine.createGameObject(2).getYlocation());
        image.setLayoutX(gameEngine.createGameObject(2).getXlocation());
    }
    ...
    private void createExtraTimeFruit(){
     if (Case==1) {   System.out.println("Number of SpecialFruit: "+ExtratimeLevel);
        if(ExtratimeLevel!=0){
            for (int i=0; i<ExtratimeLevel; i++){
            	TimeObject.add(i, (ExtraTime) gameEngine.createGameObject(2));
            	TimeObject.get(i).setSlicedFromGui(false);
            	CurrentSpecial.add(i, ((SPECIAL) TimeObject.get(i).getObjectType()));

                EXTRATIME_PATH = CurrentSpecial.get(i).getIdle();
                TEXT = CurrentSpecial.get(i).getExtraseconds();
                extratime.add(i, new ImageView(EXTRATIME_PATH));


                extratime.get(i).setFitWidth(100);
                extratime.get(i).setFitHeight(150);
                TimeObject.get(i).setIsReachedMaxHeight(false);
                TimeObject.get(i).setSlicedFromGui(false);
                setNewElementPositionSPECIAL(extratime.get(i));
                gamePane.getChildren().add(extratime.get(i));
            }
        }
    }
    }  
    private void moveElements() {
        ...
        if(ExtratimeLevel!=0){
            for (int i = 0; i < TimeObject.size(); i++) {
                if (!TimeObject.get(i).isSliced()) {
                    if (!TimeObject.get(i).getIsReachedMaxHeight()) {

                        if (extratime.get(i).getLayoutY() > gameEngine.createGameObject(2).getMaxHeight()) {
                        	TimeObject.get(i).setIsReachedMaxHeight(false);
                            extratime.get(i).setLayoutY(extratime.get(i).getLayoutY() - TimeObject.get(i).getInitialVelocity());
                            extratime.get(i).setLayoutX(extratime.get(i).getLayoutX() + 1);
                            extratime.get(i).setRotate(extratime.get(i).getRotate() + 5);
                        } else {
                        	extratime.get(i).setLayoutX(extratime.get(i).getLayoutX() + 5);
                        	TimeObject.get(i).setIsReachedMaxHeight(true);
                        }
                    } else {
                        extratime.get(i).setLayoutY(extratime.get(i).getLayoutY() + TimeObject.get(i).getInitialVelocity());
                        extratime.get(i).setLayoutX(extratime.get(i).getLayoutX() + 1);
                        extratime.get(i).setRotate(extratime.get(i).getRotate() + 5);
                        if (extratime.get(i).getLayoutY() > 705 ) {
                        	extratime.remove(extratime.get(i));
                            CurrentSpecial.remove(CurrentSpecial.get(i));
                            TimeObject.remove(TimeObject.get(i));
                            i=0;
...
//            System.out.println("GameOver");
    ...
    public void addtime() {
    	...
    	{ 
    		this.time=time;
    		this.tmp=tmp;
    		time.label.setText(time.getTmp()+""+3);
    ...
    private void ImageEVENT(){
        ...
        gamePane.setOnMouseDragged(event -> {
            ...
            for(int i=0; i<extratime.size(); i++) {
                if (Math.abs(event.getSceneX() - extratime.get(i).getLayoutX()) < 100) {
                    if (Math.abs(event.getSceneY() - extratime.get(i).getLayoutY()) < 50) {

                    	extratime.get(i).setImage(new Image(TEXT));
                        TimeObject.get(i).setSlicedFromGui(true);
                        if(!fruit.isEmpty()){
                            for(int j=0; j<fruit.size(); j++){
                                gamePane.getChildren().removeAll(fruit.get(j));
                            }
    ...
    private void TimeImage() {
        for (int i = 0; i < extratime.size(); i++) {
            if (TimeObject.get(i).isSliced()) {
                ...
                if(extratime.get(i).getFitHeight() < 200){
                	extratime.get(i).setFitHeight(extratime.get(i).getFitHeight() + 2);
                	extratime.get(i).setFitWidth(extratime.get(i).getFitWidth() + 1);
                }
                if(extratime.get(i).getFitHeight() >= 200 && extratime.get(i).getFitHeight() < 300){
                	extratime.get(i).setRotate(0);
                	extratime.get(i).setImage( new Image(TEXT));
                ...
                this.time=time;
                time.label.setText(time.getTmp()+""+3);
//                if(extratime.get(i).getFitHeight() >= 300 && extratime.get(i).getFitHeight() < 500){
//                	extratime.get(i).setImage( new Image(EXPL3));
//                	extratime.get(i).setLayoutY(extratime.get(i).getLayoutY() - 5);
//                	extratime.get(i).setFitHeight(extratime.get(i).getFitHeight() + 5);
//                    extratime.get(i).setFitWidth(extratime.get(i).getFitWidth() + 3);
//                }
//
//                if(extratime.get(i).getFitHeight() >= 500 && extratime.get(i).getFitHeight() <= 700){
//
//                	extratime.get(i).setImage( new Image(txtEXPL));
//                	extratime.get(i).setLayoutX(200);
//                	extratime.get(i).setLayoutY(50);
//                	extratime.get(i).setFitHeight(extratime.get(i).getFitHeight() + 2);
//                	extratime.get(i).setFitWidth(extratime.get(i).getFitWidth() + 5);
//                }
//                if(boom.get(i).getFitHeight() >= 700)
//                    youLostHAHA =true;
            }
    ...
    private void checkIfElementsBelowScreen() {
        ...
        for (int i = 0; i < extratime.size(); i++) {
            if (extratime.get(i).getLayoutY() >= 810) {
                CurrentSpecial.remove(CurrentSpecial.get(i));
                extratime.remove(extratime.get(i));
                TimeObject.remove(TimeObject.get(i));
            }
        }
        if(extratime.isEmpty()) {
            createExtraTimeFruit();
        }
